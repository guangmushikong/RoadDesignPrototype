...
class Route_merge():
    ...
    def readtxt(self, filename, filepath, splitstr):
        ...
        data_list = []
        fn = open(path_name, encoding='UTF-8')
        # print(path_name)
        for line in fn:
    ...
    # 计算合并文件,filelist:排序好之后的文件名，每个路线的长度字典{3：71.000}
    ...
    def pro(self):
        ...
        # print(filelist)
        print(dis_list)
        if sum(dis_list)<=int(self.ky):
            print('所有航程合并后距离小于阈值！')
            #获取文件名列表
            fullname = []
            savename = str(filelist[0])+'-'+str(filelist[-1])+'.ght'
            for val in filelist:
                fullname.append(str(val)+'.ght')
            self.ght_merge(fullname, savename)
